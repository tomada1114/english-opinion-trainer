...
- Keep a constant next to the code that reads it. `STATUS_BY_LLM_CODE` sits in
  `src/server/handlers/feedback.ts` because that handler is the only module that reads
  it. Do not create a shared `constants.ts` grab-bag that forces unrelated modules to
  import each other.
