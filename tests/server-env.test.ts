...
describe("readServerEnv", () => {
  ...
  // A credential pasted out of a secret manager often carries a trailing
  // newline; trimming it is what keeps that paste equal to the value the
  // adapter was actually issued.
  ...
  // The regression a presence-based rule elsewhere in this application once
  // caused: a machine that exports ANTHROPIC_API_KEY for something else --
  // recording the LLM fixtures under `LLM_RECORD=1` needs it -- must still be
  // able to read the environment without anything refusing to boot.
