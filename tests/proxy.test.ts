...
// prefixes render. Disagree, and `/` 404s — or `/api/feedback` gets redirected to
// `/en/api/feedback` — with every other suite still green, because nothing else in
...
describe("the paths locale detection runs on", () => {
  ...
  it.each([
    ["an API route", "/api"],
    ["a nested API route", "/api/feedback"],
