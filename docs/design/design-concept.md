...
| Design-system base | shadcn/ui + Tailwind v4                                                                                                                                                                                                                           |
| Why that base      | The chosen brand axes (playful, experimental) are cheapest to express through Tailwind's utilities. The one primitive that needed Radix's accessibility work, `Button`'s `asChild`, is now carried by a local `Slot` component instead (see #79). |
| Known cost of it   | None paid ahead of use. Radix was dropped once its one use turned out to be a single prop this app could implement locally.                                                                                                                       |
...
| 2026-09-13 | **Superseded 2026-09-14, see below.** shadcn/ui + Radix + Tailwind v4                                                                                                   | Plain CSS custom properties; Tailwind alone; Tailwind + Radix only | The author's call, made after the cost was stated: Radix is paid for before the parts that need it exist.                                                                                                                                                            |
...
| 2026-09-14 | shadcn/ui + Tailwind v4, dropping Radix                                                                                                                                 | Keeping Radix as an unused, paid-ahead-of-use dependency           | Its one use (`Button`'s `asChild`) turned out to be a single prop, implemented locally as `Slot` (#79); the cost accepted in the 2026-09-13 row never had to be spent.                                                                                               |
