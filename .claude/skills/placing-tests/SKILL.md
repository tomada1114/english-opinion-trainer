...
- **`component`** — `tests/**/*.test.tsx`, selected by the extension alone. This is the
  ...
  `tests/home-page.test.tsx` renders a synchronous Server Component and
  `tests/units-drill.test.tsx` renders the drill page's Client Component with `fetch`
  stubbed. Neither does real I/O, so both keep the same short budget as `unit`.
